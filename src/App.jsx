...
function App() {
  ...
  const formatAthDate = (dateString) => {
  ...
  return (
    ...
    <div className="min-h-screen bg-gray-50 flex flex-col items-center justify-center px-2 py-8">
      <div className="w-full max-w-xl mx-auto flex flex-col gap-8">
        {/* Card for price, 24h high/low, ATH, and chart */}
        <div className="bg-white rounded-2xl shadow-lg border border-gray-200 p-8 flex flex-col items-center mx-auto w-full max-w-xl gap-4">
          {/* App title */}
          <h1 className="text-2xl font-extrabold text-gray-900 mb-2 text-center">$TAO Price Chart</h1>
          {/* Current price, 24h range, and ATH */}
          <div className="flex flex-col sm:flex-row gap-6 mb-4 w-full justify-center items-center">
            {/* Current price */}
            <div className="text-lg font-semibold text-gray-800 min-w-[160px]">
              ...
              {priceLoading ? (
                ...
                <span className="text-indigo-600" data-testid="current-price">${currentPrice.toLocaleString()}</span>
              ) : (
                <span className="text-red-500">N/A</span>
              )}
            </div>
            {/* 24h Range */}
            <div className="text-lg font-semibold text-gray-800 min-w-[180px]">
              24h Range:{' '}
              {priceLoading ? (
                <span className="inline-block align-middle">
                  <span className="inline-block h-5 w-28 bg-gray-200 rounded animate-pulse" />
                </span>
              ) : low24h !== null && high24h !== null ? (
                <span data-testid="range-24h">
                  <span className="text-gray-600">${low24h.toLocaleString()}</span>
            ...
            <div className="text-lg font-semibold text-gray-800 min-w-[180px]">
              ...
              {priceLoading ? (
                ...
                <span data-testid="ath">
                  <span className="text-amber-600">${ath.toLocaleString()}</span>
                  {athDate && (
                    <span className="text-xs text-gray-400 ml-1">({formatAthDate(athDate)})</span>
                  )}
                </span>
              ) : (
                <span className="text-red-500">N/A</span>
